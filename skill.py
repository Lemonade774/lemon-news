...
def generate_html(items: Iterable[NewsItem], output_path: Path, updated_at: str | None = None) -> None:
    ...
    for number, item in enumerate(items, 1):
        ...
        cards.append(f'''<article class="news-item" data-news-id="{item_id}"><div class="news-number">{number:02d}</div><div class="news-content"><div class="news-title-row"><h2><a href="{safe_url}" target="_blank" rel="noopener">{html.escape(item.title)}</a></h2></div><div class="news-meta"><div class="meta-primary"><span class="score {score_class}">热度 {item.importance_score}</span><span class="source">{html.escape(item.source)}</span></div><time>{html.escape(item.publish_time)}</time></div><div class="tags">{tag_html}</div><div class="news-actions"><button class="archive-btn" type="button" data-archive-id="{item_id}" data-title="{html.escape(item.title, quote=True)}" data-url="{safe_url}" data-source="{html.escape(item.source, quote=True)}" title="归档新闻" aria-label="归档新闻">☆</button><a class="read-more" href="{safe_url}" target="_blank" rel="noopener">阅读原文 ↗</a></div><p>{html.escape(item.summary or "暂无摘要")}</p></div></article>''')
    ...
    tag_filters = '<div class="tag-filters" aria-label="按标签筛选"><button class="tag-filter is-active" type="button" data-tag="*">全部</button>' + ''.join(f'<button class="tag-filter" type="button" data-tag="{html.escape(tag, quote=True)}">{html.escape(tag)}</button>' for tag in tag_names) + '</div>'
